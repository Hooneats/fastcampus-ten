...
//        bindings.bind(root.title).first(StringExpression::likeIgnoreCase); // 대소문자 구분없이 like '${v} -> % 넣는것을 해줘야함
        ...
        bindings.bind(root.hashtag).first(StringExpression::containsIgnoreCase); // 대소문자 구분없이 like '%${v}%'
        bindings.bind(root.createAt).first(DateTimeExpression::eq); // 대소문자 구분없이 like '%${v}%'
